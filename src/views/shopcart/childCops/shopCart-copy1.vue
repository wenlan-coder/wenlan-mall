<template>
 ...
 <!--   购物车为空组件-->
  ...
  description="购物车是空的哟！"
      />
  <!-- 购物车复选框组 -->
  <van-checkbox-group v-model="checkgroupList"  ref="checkboxGroup"  style="margin-bottom:30px">
  <div v-for="(item,index) in cardList" :key="index">
    
     <van-swipe-cell>
       <!-- 自己定义添加左侧复选框 -->
     <van-checkbox :name="item" checked-color="#9381ff" class="checkbox"></van-checkbox>
  <van-card
    :price="item.price"
    :desc="item.desc"
    :title="item.title"
  ...
  <van-image :src="item.image" class="van-img" width="88px" height="88px" ></van-image>
  </div>
  </template>
  </van-card>
   <template #right>
  ...
  <van-checkbox  @click="checkAll"  v-model="isAllChecked" checked-color="#9381ff"  :disabled="isNull">全选</van-checkbox>
</van-submit-bar>
  </div>
</template>
...
import { Checkbox, CheckboxGroup,Card,SwipeCell,Button,Stepper,SubmitBar, Empty} from 'vant';
